...
def make_references_unique(rows):
    """Hace unica la referencia de cada linea para evitar duplicados en el importador."""
    counters = defaultdict(int)
    out = []

    for r in rows:
        row = dict(r)
        base_ref = str(row.get('GL_Reference', '')).strip() or 'Provision produccion'
        job = str(row.get('JobNumber', '')).strip()

        key = (
            str(row.get('GL_Account', '')).strip(),
            str(row.get('GL_Note', '')).strip(),
            str(row.get('GL_Month', '')).strip(),
            str(row.get('GL_Year', '')).strip(),
            str(row.get('GL_Group', '')).strip(),
            str(row.get('TransactionDate', '')).strip(),
            base_ref,
            job,
        )
        counters[key] += 1
        ...
        suffix_parts = []
        if job:
            suffix_parts.append(job)
        suffix_parts.append(f'{seq:03d}')

        row['GL_Reference'] = f"{base_ref} - {' - '.join(suffix_parts)}"
        out.append(row)
...
if run:
    ...
    try:
        ...
        out_rows = make_references_unique(out_rows)
