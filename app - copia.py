...
def add_auto_offsets(rows, offset_account='1300102.5', agg='total'):
    ...
    Genera contrapartidas:
      - agg == 'total': UNA por el total (Nota/Referencia auto por mes/año del archivo).
...
# ---------- AGENCIAS: Cargar base y UI ----------
def load_agencies_from_repo():
    """
    Lee 'agencias.csv' (o 'agencias.xlsx') en la raíz del repo y
    devuelve un dict {Agencia -> Cuenta}. Tolerante a nombres de columnas.
    """
    # 1) CSV
    for fname in ("agencias.csv", "agencias.CSV"):
        if os.path.isfile(fname):
            try:
                ...
                pass
    # 2) XLSX opcional
    for fname in ("agencias.xlsx", "agencias.XLSX"):
        if os.path.isfile(fname):
            try:
...
def build_agency_map(df):
    ...
    if not ag_cols or not ct_cols:
        return {}
    agencia_col = ag_cols[0]
    cuenta_col  = ct_cols[0]
    mapping = {}
    for _, r in df.iterrows():
        ag = str(r[agencia_col]).strip()
        ct = str(r[cuenta_col]).strip()
        if ag and ct and ct.lower() != "nan":
            mapping[ag] = ct
    return mapping

agency_map = load_agencies_from_repo()
...
if agency_map:
    ...
    st.info("💡 Para habilitar selector de agencias, agrega un archivo **agencias.csv** (Agencia, Cuenta) en la raíz del repo.")
