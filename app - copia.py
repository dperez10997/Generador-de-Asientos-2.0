...
def _read_agency_df(path: Path):
    try:
        if path.suffix.lower()==".xlsx": return pd.read_excel(path, dtype=str)
...
def load_agencies():
    """Busca agencias.csv/xlsx en ./, ./data, ./assets."""
...
if agency_map:
    ...
    st.info("ℹ️ Coloca **agencias.csv** (o .xlsx) en la raíz del repo con columnas: Agencia, Cuenta. "
            "También se aceptan equivalentes (Cliente/Account).")
